...
public class PermissionInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object Handler) throws IOException {
		...
		// 비로그인 && /post => 로그인 페이지로 이동, Controller 수행 방지
		if (userId == null && uri.startsWith("/post")) {
			response.sendRedirect("/user/sign-in-view");
			return false; // Controller 수행 안함 (원래 요청에 대해서 끝)
		}
		
		// 로그인 && /user => 글 목록 페이지 이동, Controller 수행 방지
	...
	public void postHandle(HttpServletRequest request, HttpServletResponse response, Object Handler, ModelAndView Mav) {
		// view 객체가 mav로 존재한다는 건 아직 jsp가 html로 변환되기 전이다.
